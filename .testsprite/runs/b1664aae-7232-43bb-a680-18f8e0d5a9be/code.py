...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Daftar Arsip' link in the sidebar.
        # Daftar Arsip link
        elem = page.get_by_role('link', name='Daftar Arsip', exact=True)
        await elem.click(timeout=10000)
        
        # -> Click the 'Peminjaman' link in the sidebar to open the loan form page.
        # Peminjaman link
        elem = page.get_by_role('link', name='Peminjaman', exact=True)
        await elem.click(timeout=10000)
        
        # -> Fill the 'Nomor Warkah' field with '1234/2026', enter a borrower name in 'Nama Peminjam', add a note in 'Keperluan / Catatan', then click the 'Simpan Peminjaman' button.
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Permintaan pinjam untuk penelitian internal.")
